...
size_t Domerau(const string &str1, const size_t len1, const string &str2, const size_t len2, size_t **matr) {
    if (matr[len1][len2] != 0) {
        ...
        bool flag = str1[len1 - 1] != str2[len2 - 1];
        matr[len1][len2] = min(min(Domerau(str1, len1 - 1, str2, len2, matr) + 1,
                                   Domerau(str1, len1, str2, len2 - 1, matr) + 1),
                               Domerau(str1, len1 - 1, str2, len2 - 1, matr) + flag);

        if (len1 > 1 && len2 > 1 && str1[len1 - 2] == str2[len2 - 1] && str1[len1 - 1] == str2[len2 - 2]) {
            matr[len1][len2] = min(Domerau(str1, len1 - 2, str2, len2 - 2, matr) + 1,
                                   matr[len1][len2]);
        }
    }
    return matr[len1][len2];
...
size_t Domerau_method(const string &str1, const string &str2) {
    ...
    Domerau(str1, len1, str2, len2, matr);
